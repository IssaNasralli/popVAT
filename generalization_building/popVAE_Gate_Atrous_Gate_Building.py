...
def create_popVAT_Building(patch_size, patch_size_global, latent_dim, bands, atrous):     
    ...
    popVAT_Building_model = Model(inputs=[xc, xg, input_xp, y_true], outputs=[vae_output, p_pred], name='popVAT_Building')
    popVAT_Building_model.add_loss(vae_loss_layer)
    ...
    popVAT_Building_model.compile(optimizer=optimizer, loss=None)

    return popVAT_Building_model
